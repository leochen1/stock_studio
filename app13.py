...
def analyze_stock(stock_code):
    ...
    kd = talib.STOCH(df.High, df.Low, df.Close, fastk_period=9, slowk_period=3, slowd_period=3)  # 計算KD指標
    ...
    if last_row['+DI_increase'] and last_row['-DI_decrease'] and last_row['ADX_increase'] and last_row['+DI_greater_than_-DI'] and last_row['ADX_less_than_25'] and last_row['Volume_increase'] and last_row['Volume_1000'] > 1000 and last_row['Close_less_than_100'] < 100 and last_row['Close_greater_than_SMA20'] and last_row['RSI_14_greater_than_50'] and last_row['RSI_Pre_Gold_Cross']:
...
class CrossStrategy(Strategy):
    def init(self):
        ...
        self.rsi_10 = self.I(talib.RSI, self.data.Close, timeperiod=10)
        self.k, self.d = self.I(talib.STOCH, self.data.High, self.data.Low, self.data.Close)

    def next(self):
        if crossover(self.rsi_5, self.rsi_10):
            self.buy()
        elif crossover(self.k, self.d) and self.d[-1] < 50:
            self.sell()

def backtest_stocks(stock):
    print("stock: ", stock)
    data = yf.download(stock, period="5y")
    data['Symbol'] = stock

    backtest = Backtest(data, CrossStrategy, cash=100000, commission=0.004)
    stats = backtest.run()
...
if __name__ == "__main__":
    # main()
    matched_stocks = ['1402.TW','2324.TW','6005.TW','8088.TWO']
    for stock in matched_stocks:
        backtest_stocks(stock)